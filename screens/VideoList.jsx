            ...
            <View className=' flex justify-start items-center flex-col w-screen'>
              {videos.length != 0 ?
                videos.map((item, index) => (
                  <TouchableOpacity className='w-[90vw] bg-white p-4 py-2 mb-2 rounded-md flex justify-between flex-row items-center' key={index}
                    onPress={() => Linking.openURL(item)}
                  >
                      <Text className='text-slate-600 w-[90%]'>{item}</Text>
                      <TouchableOpacity onPress={() => copyToClipboard(item)} className='bg-slate-200 p-2 rounded-md'>
